...
Tile.write = function (obj, pbf) {
  if (obj.layers)
    for (let i = 0; i < obj.layers.length; i++) pbf.writeMessage(3, Tile.Layer.write, obj.layers[i]);
};
...
Tile.Layer.write = function (obj, pbf) {
  ...
  if (obj.features)
    for (var i = 0; i < obj.features.length; i++) pbf.writeMessage(2, Tile.Feature.write, obj.features[i]);
  if (obj.keys)
    for (i = 0; i < obj.keys.length; i++) pbf.writeStringField(3, obj.keys[i]);
  if (obj.values)
    for (i = 0; i < obj.values.length; i++) pbf.writeMessage(4, Tile.Value.write, obj.values[i]);
